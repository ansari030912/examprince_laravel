...
@section('main-content')
            ...
            <div class="flex flex-wrap -m-4 justify-center">
                <div class="w-full lg:w-8/12 p-4">
                    <div class="px-4 py-6 border border-gray-200 rounded-2xl">
                        ...
                        <!-- This container will be updated dynamically with cart items -->
                        <div id="orderItems">
                    ...
                    <div id="checkoutDetails">
                        ...
                        <div class="py-6 border-b border-dashed">
                            ...
                            <div class="flex flex-wrap gap-4 mb-4">
                                <input class="py-3 px-4 w-full text-sm placeholder-gray-500 bg-gray-50 outline-none focus:ring focus:ring-gray-100 border-gray-100 rounded-lg transition duration-200" placeholder="Full Name" type="text" name="fullName">
                                <input class="py-3 px-4 w-full text-sm placeholder-gray-500 bg-gray-50 outline-none focus:ring focus:ring-gray-100 border-gray-100 rounded-lg transition duration-200" placeholder="Email" type="text" name="email">
                            </div>
                            <h6 class="mb-4 text-lg font-semibold">Discount Code</h6>
                            <form method="POST">
                                @csrf
                                <div class="flex items-center gap-2">
                                    <input class="py-3 px-4 w-full text-sm placeholder-gray-500 bg-gray-50 outline-none focus:ring focus:ring-gray-100 border-gray-100 rounded-lg transition duration-200" placeholder="Enter your voucher" type="text" name="coupon" value="{{ old('coupon', 'MEGASALE') }}">
                                    <button type="submit" class="py-3 px-7 w-full lg:w-auto text-sm text-white font-semibold bg-gray-900 hover:bg-gray-800 focus:bg-gray-900 rounded-5xl focus:ring-4 focus:ring-gray-200 transition duration-300">Apply</button>
                                </div>
                            </form>
                            @if (session('message'))
                                <p class="mt-4 text-sm font-semibold text-green-600 text-center">{{ session('message') }}</p>
                            @endif
                            <p class="text-sm font-semibold text-green-600">Coupon <span class="text-red-500 font-bold">'MEGASALE'</span> is Applied Successfully.</p>
                        </div>
                        ...
                        <label class="flex items-center">
                            ...
                            <span>I agree to the <a class="text-blue-400" href="/terms-and-conditions">terms and conditions</a></span>
                        </label>
                        <button class="py-3 px-7 mt-6 w-full text-sm text-white font-semibold bg-blue-500 hover:bg-blue-600 focus:bg-blue-500 rounded-5xl focus:ring-4 focus:ring-gray-200 transition duration-300">Check Out</button>
                    </div>
                </div>
            </div>
        </div>
    ...
    <!-- Script to fetch cart data, update the order summary, handle deletion, recalc totals, and conditionally show checkout details -->
    <script>
        // Function to update the order summary from localStorage
                ...
                fetch("{{ route('cart.process') }}", {
                    ...
                    .then(data => {
                        ...
                        if (data.cartData && data.cartData.length > 0) {
                            ...
                            // Calculate discount and off percentage
                    ...
                    .catch(error => {
                        ...
                        document.getElementById('orderItems').innerHTML = "<p class='text-center text-red-500'>Failed to load order details.</p>";
                    });
            } else {
                document.getElementById('orderItems').innerHTML = "<p class='text-center text-gray-500'>No cart data found in localStorage.</p>";
                // Hide checkout details if no cart data
                checkoutDetails.style.display = "none";
            }
        }
        ...
        function attachDeleteListeners() {
            ...
            deleteButtons.forEach(button => {
                button.addEventListener('click', function() {
                    ...
                    // Filter out the item that matches the clicked id
        ...
        // On DOM content loaded, update the order summary
